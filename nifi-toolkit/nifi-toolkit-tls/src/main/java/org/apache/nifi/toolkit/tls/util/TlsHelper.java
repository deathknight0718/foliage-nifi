...
public class TlsHelper {
    ...
    public static byte[] calculateHMac(String token, PublicKey publicKey) throws GeneralSecurityException {
        ...
        if (tokenBytes.length < 16) { throw new GeneralSecurityException("Token does not meet minimum size of 16 bytes."); }
    ...
    public static JcaPKCS10CertificationRequest parseCsr(String pemEncodedCsr) throws IOException {
        try (PEMParser pemParser = new PEMParser(new StringReader(pemEncodedCsr))) {
            ...
            if (!PKCS10CertificationRequest.class.isInstance(o)) { throw new IOException("Expecting instance of " + PKCS10CertificationRequest.class + " but got " + o); }
    ...
    public static <T> T parsePem(Class<T> clazz, Reader pemReader) throws IOException {
        try (PEMParser pemParser = new PEMParser(pemReader)) {
            ...
            if (!clazz.isInstance(object)) { throw new IOException("Expected " + clazz + " but got " + object.getClass()); }
            return (T) object;
        }
    }
    ...
    public static Extensions createDomainAlternativeNamesExtensions(String domainAlternativeNames, String requestedDn) throws IOException {
        ...
        if (StringUtils.isNotBlank(domainAlternativeNames)) {
            for (String alternativeName : domainAlternativeNames.split(",")) {
                // @see: https://docs.oracle.com/javase/7/docs/technotes/tools/windows/keytool.html
                if (StringUtils.startsWith(alternativeName, "dns:")) namesList.add(new GeneralName(GeneralName.dNSName, StringUtils.removeStart(alternativeName, "dns:")));
                else if (StringUtils.startsWith(alternativeName, "ip:")) namesList.add(new GeneralName(GeneralName.iPAddress, StringUtils.removeStart(alternativeName, "ip:")));
                else namesList.add(new GeneralName(GeneralName.dNSName, alternativeName));
